...
REVOKE ALL ON FUNCTION public.reset_tournament_live_state(uuid, integer, timestamp with time zone) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.reset_tournament_live_state(uuid, integer, timestamp with time zone) TO authenticated, service_role;